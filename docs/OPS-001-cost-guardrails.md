...
**Related**: ARCH-001, REQ-001 §7, ADR-012, ADR-013, ADR-018, ADR-020
...
| **$20 alarm** | Pull **lever 1** (scheduled shutdown, ARCH-001) immediately. Do not wait for the month to close | same day |
| Month closes **above 30,000 KRW or above USD 21.4** | Write an ADR proposing lever 2 (Reserved Instance) or a design change. The ceiling is not adjusted to fit the spend | first week of the next month |
| **60 days before credit expiry** | Re-run the cost model with **measured** Cost Explorer figures, decide lever 1 vs lever 2, and record the decision. This is a calendar item, not a reaction | 60 days out |
...
This section implements REQ-001 NFR-6. All three mechanisms are required; see ADR-012 for
why the third cannot be replaced by the first two.

| # | Mechanism | Fires when | Channel |
|---|---|---|---|
| A | Failure alert | latest `collection_runs.status = 'FAILED'` (**N = 1**) | Discord webhook, from the collector |
| B | Staleness alert | `now() − last_success > interval × 1.5` | Discord webhook, from a VictoriaMetrics alert rule |
| C | Dead man's switch | no ping received within the 30-hour grace period | healthchecks.io → Discord, **from outside the cluster** |

Discord was chosen over email because it needs no domain verification and costs nothing.
